    ...
    def pre_dit(
        ...
        if temporal_position_ids is not None:
            ...
            if temporal_ids.ndim == 1:
                ...
                if (
                    int(temporal_ids.shape[0]) != batch_size
                    or int(temporal_ids.shape[1]) != f
                ):
                    raise ValueError(
                        "`temporal_position_ids` must be 1D [F] or 2D [B,F], "
                ...
                raise ValueError(
                    "`temporal_position_ids` must be 1D [F] or 2D [B,F], "
                    f"got shape {tuple(temporal_ids.shape)}."
                )
            ...
            if temporal_offset < 0:
                raise ValueError(
                    f"`temporal_position_offset` must be >= 0, got {temporal_offset}"
                )
        ...
        if temporal_freqs.ndim == 2:
            ...
            temporal_freqs = temporal_freqs.view(
                batch_size, f, 1, 1, -1
            ).expand(batch_size, f, h, w, -1).reshape(batch_size, f * h * w, -1)
            freqs = torch.cat([
                temporal_freqs,
                spatial_freqs.expand(batch_size, -1, -1),
            ], dim=-1).unsqueeze(2).to(x_tokens.device)
